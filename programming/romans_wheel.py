...
while (1):
    ...
    if text.find('PING') != -1:
        ...
        irc_command("PRIVMSG %s : !ep3" % BOTNAME)
    elif text.find(BOTNAME) != -1 and text.find("PRIVMSG %s" % NICKNAME) != -1 and text.find("Bad") == -1 and text.find("password") == -1:
        # detect and parse bot answer
        rot13string = text.split(':')[2].split('\r')[0]
        print(rot13string)
        
        result = ""
        for c in rot13string:
            if c <= '9':
                rot13 = ord(c)
            elif c >= 'a':
                rot13 = ord(c) + 13
        ...
        irc_command("PRIVMSG %s : !ep3 -rep %s" % (BOTNAME, result))
